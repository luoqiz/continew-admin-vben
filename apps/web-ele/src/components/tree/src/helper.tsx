 ...
 */
export function menusWithPermissions(menus: MenuResp[]) {
  eachTree(menus, (item: MenuPermissionOption) => {
    validateMenuTree(item);
    // 将第一级的菜单作为主菜单，子级作为权限控制
...
export function setTableChecked(
  ...
  if (!association) {
    checkedRows = checkedRows.filter(
      (item) => isNil(item.children) || isEmpty(item.children),
    );
  }
 ...
 * @param menu menu
 */
function validateMenuTree(menu: MenuResp) {
  /**
  ...
  if (menu.type === 2) {
    menu.children?.forEach?.((item) => {
      if ([1].includes(item.type)) {
        const description = `错误用法: [${menu.title} - 菜单]下不能放 目录 -> [${item.title}]`;
  ...
  if (menu.type === 3) {
    ...
    menu.children?.forEach?.((item) => {
      if ([1, 2, 3].includes(item.type)) {
        const description = `错误用法: [${menu.title} - 按钮]下不能放置'目录/菜单/按钮' -> [${item.title}]`;
        console.warn(description);
        notification.warning({
          title: '提示',
          message: description,
