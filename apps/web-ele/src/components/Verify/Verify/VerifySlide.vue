...
import { encryptByAes, resetSize } from '@vben/utils';

import { checkBehaviorCaptcha, getBehaviorCaptcha } from '#/api/common/captcha';
      ...
      if (status.value && isEnd.value === false) {
        ...
        if (
          ...
          barArea.value.offsetWidth -
            Number.parseInt(blockSize.value.width, 10) / 2 -
            2
        ) {
          move_block_left =
            barArea.value.offsetWidth -
    ...
    function end() {
      ...
      if (status.value && isEnd.value === false) {
        ...
        checkBehaviorCaptcha(data).then((res) => {
          if (res.success && res.repCode === '0000') {
