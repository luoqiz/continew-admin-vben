...
import type { ClientResp, ClientQuery } from '#/api'
...
import { listClient, deleteClient } from '#/api'

import { useClientGridFieldColumns, useClientGridSearchFormSchema } from './ClientData'
import ClientEditDrawer from './AddModal.vue'

const [TableGrid, tableGridApi] = useVbenVxeGrid({
  formOptions: { schema: useClientGridSearchFormSchema(), submitOnChange: true, showCollapseButton: false },
  gridOptions: {
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async ({ page }, formValues) => {
          const res = await listClient({ page: page.currentPage, size: page.pageSize, ...formValues } as ClientQuery)
          return res
        },
      },
    },
...
const [EditorDrawer, drawerApi] = useVbenDrawer({ connectedComponent: ClientEditDrawer, destroyOnClose: true })
  ...
  <Page auto-content-height>
    <TableGrid :table-title="$t('system.client.listTitle') || '客户端配置'">
      ...
      <template #isConcurrent="{ row }">
        <ElTag v-if="row.isConcurrent" type="success">{{ $t('common.yes') }}</ElTag>
