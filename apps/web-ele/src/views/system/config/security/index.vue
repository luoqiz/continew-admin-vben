...
import { type OptionResp, type SecurityConfig, listOption, resetOptionValue, updateOption } from '#/api/system'
...
const onResetValue = () => {
  ElMessageBox.confirm('确认恢复安全配置为默认值吗？',
    '警告', {
...
<template>
  <div v-loading="loading">
    <el-form ref="formRef" :model="form" :rules="rules" auto-label-width label-align="left" label-width="180px"
      :layout="width >= 500 ? 'horizontal' : 'vertical'" :disabled="!isUpdate" scroll-to-first-error class="form">
      <el-form-item field="PASSWORD_ERROR_LOCK_COUNT" :label="securityConfig.PASSWORD_ERROR_LOCK_COUNT?.name"
        :help="securityConfig.PASSWORD_ERROR_LOCK_COUNT?.description" hide-asterisk>
        <el-input-number v-model="form.PASSWORD_ERROR_LOCK_COUNT" :default-value="0" :precision="0" :min="0" :max="10">
          <template #suffix>次</template>
        </el-input-number>
      </el-form-item>
      <el-form-item field="PASSWORD_ERROR_LOCK_MINUTES" :label="securityConfig.PASSWORD_ERROR_LOCK_MINUTES.name"
        :help="securityConfig.PASSWORD_ERROR_LOCK_MINUTES.description" hide-asterisk>
        <el-input-number v-model="form.PASSWORD_ERROR_LOCK_MINUTES" :precision="0" :min="1" :max="1440">
          <template #suffix>分钟</template>
        </el-input-number>
      </el-form-item>
      <el-form-item field="PASSWORD_EXPIRATION_DAYS" :label="securityConfig.PASSWORD_EXPIRATION_DAYS.name"
        :help="securityConfig.PASSWORD_EXPIRATION_DAYS.description" hide-asterisk>
        <el-input-number v-model="form.PASSWORD_EXPIRATION_DAYS" :precision="0" :min="0" :max="999">
          <template #suffix>天</template>
        </el-input-number>
      </el-form-item>
      <el-form-item :label="securityConfig.PASSWORD_EXPIRATION_WARNING_DAYS.name"
        field="PASSWORD_EXPIRATION_WARNING_DAYS" :help="securityConfig.PASSWORD_EXPIRATION_WARNING_DAYS.description"
        hide-asterisk>
        <el-input-number v-model="form.PASSWORD_EXPIRATION_WARNING_DAYS" :precision="0" :min="0" :max="998">
          <template #suffix>天</template>
        </el-input-number>
      </el-form-item>
      <el-form-item field="PASSWORD_REPETITION_TIMES" :label="securityConfig.PASSWORD_REPETITION_TIMES.name"
        :help="securityConfig.PASSWORD_REPETITION_TIMES.description" hide-asterisk>
        <el-input-number v-model="form.PASSWORD_REPETITION_TIMES" :precision="0" :min="3" :max="32">
          <template #suffix>次</template>
        </el-input-number>
      </el-form-item>
      <el-form-item field="PASSWORD_MIN_LENGTH" :label="securityConfig.PASSWORD_MIN_LENGTH.name"
        :help="securityConfig.PASSWORD_MIN_LENGTH.description" hide-asterisk>
        <el-input-number v-model="form.PASSWORD_MIN_LENGTH" :precision="0" :min="8" :max="32" />
      </el-form-item>
      <el-form-item field="PASSWORD_ALLOW_CONTAIN_USERNAME" :label="securityConfig.PASSWORD_ALLOW_CONTAIN_USERNAME.name"
        ...
        <el-switch v-model="form.PASSWORD_ALLOW_CONTAIN_USERNAME" type="round" active-value="1" inactive-value="0"
          active-text="是" inactive-text="否" inline-prompt />

      </el-form-item>
      <el-form-item field="PASSWORD_REQUIRE_SYMBOLS" :label="securityConfig.PASSWORD_REQUIRE_SYMBOLS.name"
        ...
        <el-switch v-model="form.PASSWORD_REQUIRE_SYMBOLS" type="round" active-value="1" inactive-value="0"
          active-text="是" inactive-text="否" inline-prompt />
      </el-form-item>
    </el-form>
    <el-space style="margin-bottom: 16px">
      <el-button v-if="!isUpdate" v-access:code="['system:securityConfig:update']" type="primary" @click="onUpdate">
        <template #icon>
          <SvgEditIcon />
        </template>修改
      </el-button>
      <el-button v-if="!isUpdate" v-access:code="['system:securityConfig:update']" @click="onResetValue">
        <template #icon>
          <SvgUndoIcon />
        </template>恢复默认
      </el-button>
